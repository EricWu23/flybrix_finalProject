...
Control::PIDParameters::PIDParameters()
	...
	pitch_master{ 0.0, 0.0, 0.0, 300.0, 0.005, 0.005, 30.0 },
	...
	roll_gain{ 2047.0 },
	yaw_gain{ 2047.0 },
	pid_bypass{ BYPASS_THRUST_MASTER | BYPASS_THRUST_SLAVE | BYPASS_YAW_MASTER | BYPASS_PITCH_SLAVE | BYPASS_ROLL_SLAVE | BYPASS_YAW_SLAVE }  // AHRS/Horizon mode // LJD added
{
}
...
Control::Control(const PIDParameters& config, const VelocityPIDParameters& velocity_config)
	...
	//	  forward_pid(config.pitch_master, config.pitch_slave),
...
void Control::parseConfig() {
	...
	forward_pid.use<0>(false);
	forward_pid.use<1>(false);
	forward_pid.use<2>(((pid_parameters.pid_bypass & 0x2) == 0));
	forward_pid.use<3>(false);

	right_pid.use<0>(false);
	right_pid.use<1>(false);
	right_pid.use<2>(((pid_parameters.pid_bypass & 0x4) == 0));
	right_pid.use<3>(false);
...
ControlVectors Control::calculateControlVectors(const Vector3<float>& velocity, const Kinematics& feedback, const RcCommand& setpoint) {
	...
	forward_pid.pid<2>().setFeedback(radToDeg(feedback.rate.pitch));
	forward_pid.pid<3>().setInput(radToDeg(feedback.rate.pitch)); // disabled

	// Right PID Inputs
	right_pid.pid<2>().setInput(radToDeg(feedback.angle.roll));
	right_pid.pid<2>().setFeedback(radToDeg(feedback.rate.roll));
	right_pid.pid<3>().setInput(radToDeg(feedback.rate.roll)); // disabled

	// Yaw PID inputs;
	yaw_pid.pid<0>().setInput(radToDeg(feedback.angle.yaw));
	//yaw_pid.pid<0>().setInput(radToDeg(feedback.rate.yaw));		// LJD
	yaw_pid.pid<1>().setInput(radToDeg(feedback.rate.yaw));

	// Velocity PID inputs
	forward_pid.pid<1>().setInput(velocity.x);  // disabled
	right_pid.pid<1>().setInput(velocity.y);	// disabled
	up_pid.pid<1>().setInput(velocity.z);		// disabled
	...
	// Reset integegrators if throttle is in low condition
